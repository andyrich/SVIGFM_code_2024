...
Created on Thu Oct  3 15:25:13 2024
    ...
    DEVELOPMENT NOTES
        ...
        ` Still gathering earlier observation data for gauges like kenwood and agua caliente
...
def ProcessSFROutputFile(filepath): # process a file 
    data = []
    current_period = None
    ...
    with open(filepath, 'r') as file:
        for line in file:
            ...
            # Check if the line is data (contains numbers and follows the format)
            data_match = re.match(r'\s*\d+', line)
            if data_match and current_period is not None:
                # Split the line into fields (assuming fixed-width format or space-separated values)
    ...
    # Adjust the column names to match the data structure (based on your file)    # you may need to chagne this based on what prints from your file
...
# print information for By-reach Stats
